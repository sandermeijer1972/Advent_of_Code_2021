...
const newInput = changeInput(input);

// const usefulInputX = (array) => {
//     const sameX = array.filter(item => (item.split(",")[0] == item.split(",")[2]));
//     console.log(sameX);    
//     return sameX;
// };

// const usefulInputY = (array) => {    
//     const sameY = array.filter(item => (item.split(",")[1] == item.split(",")[3]));
//     console.log(sameY);
//     return sameY;
// };

// const arrayX = usefulInputX(newInput);
// const arrayY = usefulInputY(newInput);

// const positionX = (array) => {
//     const newArray = [];
//     const doubleArray = [];
//     const extraDouble = [];
//     array.forEach(item => {
//         const itemX = item.split(",")[0];
//         const itemYStart = parseInt(item.split(",")[1]);
//         const itemYEnd = parseInt(item.split(",")[3]);
//         if (itemYEnd > itemYStart) {
//             for (i = itemYStart; i <= itemYEnd; i++) {
//                 const pos = itemX + "," + i;
//                 const check = doubleArray.find(coordi => coordi === pos);
//                 if(check) {
//                     extraDouble.push(pos);
//                 } else {
//                     doubleArray.push(pos);
//                 };                  
//             };
//         } else {
//             for (i = itemYEnd; i <= itemYStart; i++) {
//                 const pos = itemX + "," + i;
//                 const check = doubleArray.find(coordi => coordi === pos);
//                 if(check) {
//                     extraDouble.push(pos);
...
//         };        
//     });
//     console.log("lengte dubbelen: " + extraDouble.length);
//     console.log("lengte lange array: " + doubleArray.length);
//     const allCoordinatesOnlyOnce = [...new Set(extraDouble)];
//     console.log("aantal dubbele: " + allCoordinatesOnlyOnce.length);
//     doubleArray.forEach(dubbele => {
//         const removeDouble = allCoordinatesOnlyOnce.find(posit => posit === dubbele);
//         if(!removeDouble) {
//             newArray.push(dubbele);
//         };
//     });
...
// const positionY = (array) => {
//     const newArray = [];
//     const doubleArray = [];
//     const extraDouble = [];
//     array.forEach(item => {
//         const itemY = item.split(",")[1];
//         const itemXStart = parseInt(item.split(",")[0]);
//         const itemXEnd = parseInt(item.split(",")[2]);
//         if (itemXEnd > itemXStart) {
//             for (i = itemXStart; i <= itemXEnd; i++) {
//                 const pos = i + "," + itemY;
//                 const check = doubleArray.find(coordi => coordi === pos);
//                 if(check) {
//                     extraDouble.push(pos);
...
// const testX = ["3,1,3,5", "3,4,3,7", "3,5,3,9"];
// const testY = ["2,5,4,5", "3,5,6,5", "1,4,3,4"];

// const allX = positionX(testX);
// const allY = positionY(testY);

// const inputAllX = positionX(arrayX); //1946 dubbel
// const inputAllY = positionY(arrayY); //2708 dubbel

// const checkDouble = (posx, posy) => {
//     const newArray = [];
//     posx.forEach(item => {
//         const found = posy.find(position => position === item);
//         if (found) {
//             newArray.push(item);
//         };
//     });
//     console.log(newArray);
//     console.log(newArray.length);
//     const allesEen = [...new Set(newArray)];
//     console.log(allesEen.length);
//     return allesEen.length;
// };

// checkDouble(allX, allY);
// //checkDouble(inputAllX, inputAllY);

// const dubbelXY = checkDouble(inputAllX, inputAllY);

// //console.log("antwoord: " + (1946 + 2708 + parseInt(dubbelXY)));

// PART 2

const makeCoordinates = (array) => {
    const coordinates = [];
    array.forEach(item => {
        if (item.split(",")[0] == item.split(",")[2]) {
            const itemX = item.split(",")[0];
            const itemYStart = parseInt(item.split(",")[1]);
            const itemYEnd = parseInt(item.split(",")[3]);
            if (itemYEnd > itemYStart) {
        ...
        } else if (item.split(",")[1] == item.split(",")[3]) {
            const itemY = item.split(",")[1];
            const itemXStart = parseInt(item.split(",")[0]);
            const itemXEnd = parseInt(item.split(",")[2]);
            if (itemXEnd > itemXStart) {
            ...
            if (itemXEnd > itemXStart) {
                if (itemYEnd > itemYStart) {
                    for(n = 0; n <= (itemXEnd - itemXStart); n++) {
                        const pos = (itemXStart+n) + "," + (itemYStart+n);
                        coordinates.push(pos);
                    };
                } else {
                    for(n = 0; n <= (itemXEnd - itemXStart); n++) {
                ...
                if (itemYEnd > itemYStart) {
                    for(n = 0; n <= (itemXStart - itemXEnd); n++) {
                        const pos = (itemXStart-n) + "," + (itemYStart+n);
                        coordinates.push(pos);
                    };
                } else {
...
const allCoordinates = makeCoordinates(newInput);

const onlyOne = [...new Set(allCoordinates)];
console.log(onlyOne.length);

const checkDubbele = (alles, eentje) => {
    const doubles = [];
    eentje.forEach(item => {
        const newArray = alles.filter(coordinaat => coordinaat == item);
...
// const test = ["0,9 -> 5,9", "8,0 -> 0,8", "9,4 -> 3,4", "2,2 -> 2,1", "7,0 -> 7,4", "6,4 -> 2,0", "0,9 -> 2,9", "3,4 -> 1,4", "0,0 -> 8,8", "5,5 -> 8,2"];

// const testInput = changeInput(test);

// const testCoordinates = makeCoordinates(testInput);

// const onlyOneTest = [...new Set(testCoordinates)];

// checkDubbele(testCoordinates, onlyOneTest);

checkDubbele(allCoordinates, onlyOne);