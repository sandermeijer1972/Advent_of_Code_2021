...
const makeVerschilArray = (array) => {
    const verschilArray = [];
    array.forEach(number => {
        const numberToCompare = number;
        let differences = [];
        for(i = 0; i < array.length; i++) {
            const verschil = Math.abs(numberToCompare - array[i]);
            //console.log(verschil);
            differences.push(verschil);            
        };
        //console.log(differences);
        const totaal = differences.reduce((accumulator, currentValue) => accumulator + currentValue);
        //console.log(totaal);
        verschilArray.push(totaal);
    });
    //console.log(verschilArray);
    const lowestTotalDifference = Math.min(...verschilArray);
    console.log("minste verschil: " + lowestTotalDifference);
    const numberWithLowestDifference = verschilArray.indexOf(lowestTotalDifference);
    console.log("bij hoogte: " + numberWithLowestDifference);
};

makeVerschilArray(testInput);

makeVerschilArray(input);


// PART 2

const verbruik = (number) => {
    let optellen = [];
    for(n = 0; n <= number; n++) {
        optellen.push(n);
    };
    const opgeteld = optellen.reduce((accumulator, currentValue) => accumulator + currentValue);
    return opgeteld;
};

// console.log(verbruik(4));
// console.log(verbruik(5));
...
const makeDifferenceArray = (array) => {
    ...
    workArray.forEach(number => {
        ...
        for(i = 0; i < array.length; i++) {
            const verschil = Math.abs(numberToCompare - array[i]);
            //console.log(verschil);
            const fuel = verbruik(verschil);
            //console.log(fuel);
            differences.push(fuel);             
        };
        //console.log(differences);
        const totaal = differences.reduce((accumulator, currentValue) => accumulator + currentValue);
        //console.log(totaal);
